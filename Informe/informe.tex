...
Luego de que la especificación pase el control de tipos, y sea analizada por el VCG y las condiciones de verificación se hayan consultado y descargado, se generan casos de prueba utilizando \setlog-TTF para todas las operaciones.

\subsection*{Operación \textbf{newProcess}}

Se inicializa \setlog-TTF y, en primer lugar, se aplica la táctica \textit{Disjunctive Normal Form (DNF)}, que lleva la operación a su forma normal disyuntiva. Esto particiona la operación de acuerdo a las precondiciones de las suboperaciones que la definen, donde \textit{newProcess\_dnf\_1} corresponde a \textit{newProcessOk}, \textit{newProcess\_dnf\_2} corresponde a \textit{newProcessOkEmptyQueue} y las tres restantes corresponden a las tres alternativas de \textit{newProcessError}.
Luego, se aplica la táctica de partición estándar (\textit{Standard Partition (SP)}) de $\cup$ sobre la operación de unión de conjuntos en la suboperación \textit{newProcessOk}. 
...
La especificación de cada rama del árbol de prueba y su caso de prueba asociado se encuentran en el archivo \verb|planificador_newProcess-tt.slog|.

\subsection*{Operación \textbf{dispatch}}

Se inicializa \setlog-TTF y, en primer lugar, se aplica la táctica \textit{Disjunctive Normal Form (DNF)}, que lleva la operación a su forma normal disyuntiva. Esto particiona la operación de acuerdo a las precondiciones de las suboperaciones que la definen, en este caso una por cada suboperación que define \textit{dispatch}, que son 5. 
Luego, se aplican las tácticas de partición estándar (\textit{Standard Partition (SP)}) de $\cup$ en la suboperación \textit{timeout} y de $\setminus$ en la suboperación \textit{dispatchProc}. Por último, se aplica la táctica \textit{Integer Intervals (II)} sobre la variable de estado \textit{RemTicks}. 
...
La especificación de cada rama del árbol de prueba y su caso de prueba asociado se encuentran en el archivo \verb|planificador_dispatch-tt.slog|.

\subsection*{Operación \textbf{terminateProc}}

Se inicializa \setlog-TTF y se aplica la táctica \textit{Disjunctive Normal Form (DNF)}, que lleva la operación a su forma normal disyuntiva. Esto particiona la operación de acuerdo a las precondiciones de las suboperaciones que la definen, donde \textit{terminateProc\_dnf\_1} corresponde a \textit{terminateProcOk} y \textit{terminateProc\_dnf\_2} corresponde a \textit{terminateProcError}. 
Luego de aplicar DNF, notamos que no es posible aplicar otra táctica sobre esta operación ya que no se realizan operaciones significativas dentro de las suboperaciones.
...
La especificación de cada rama del árbol de prueba y su caso de prueba asociado se encuentran en el archivo \verb|planificador_terminateProc-tt.slog|.
